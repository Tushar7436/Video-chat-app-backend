...
const roomHandler = (socket: Socket) => {
    ...
    const createRoom = () => {
        const roomId = UUIDv4(); // this will be our uinque room id in which multiple connection will exchange data
        socket.join(roomId); // we will make the socket connection enter a new room
        socket.emit("room-created", { roomId }); // we will emit an event fromserver side that socket connection has been added to a room
